...
exports.analyzeLegalDoc = onCall(
  { secrets: [ANTHROPIC_API_KEY], region: "asia-northeast3", timeoutSeconds: 120, memory: "512MiB" },
  async (request) => {
    if (!request.auth) {
    ...
    if (!docId) {
      throw new HttpsError("invalid-argument", "docId가 필요합니다.");
    }
    ...
    // Storage에서 파일 다운로드 후 base64 인코딩
    ...
    if (!isPdf) {
      // PDF가 아닌 경우(doc/docx 등)는 안내만 반환
      return {
        analysis: "현재 자동분석은 PDF 파일만 지원합니다. doc/docx 파일은 PDF로 변환 후 다시 업로드해주세요."
      };
    }
